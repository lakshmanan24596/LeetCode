...
/*
    1) DP: O(n^2) time and O(n) space
       logic: for each element, check with all prev element
       
    2) Greedy: O(n) time and O(1) space
       logic: check with prev element alone
*/
class Solution 
{
    public int wiggleMaxLength(int[] nums) 
    {
        ...
        for(int i = 1; i < nums.length; i++)
        {
            ...
            {
                // one time processing
                if(nums[i] != nums[i-1]) {  // find first non-duplicate element
                    output++;
