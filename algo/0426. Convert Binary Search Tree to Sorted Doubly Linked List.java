...
class Solution {
    ...
    public void treeToDoublyListUtil(Node curr) {
        if (curr == null) {
            return;
        }
        treeToDoublyListUtil(curr.left);
        if (prev == null) {
            head = curr;
        } else {
            prev.right = curr;      // main logic
            curr.left = prev;       // main logic
        }
        prev = curr;
        treeToDoublyListUtil(curr.right);
    }
}