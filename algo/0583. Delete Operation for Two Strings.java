...
class Solution 
{
    public int minDistance(String word1, String word2)  // Time: n*m, Space: n*m
    {
        int m = word1.length();
        int n = word2.length();
        ...
        for(int i = 1; i <= m; i++)
        {
            for(int j = 1; j <= n; j++)
            {
                if(word1.charAt(i-1) == word2.charAt(j-1)) {
                    ...
                    DP[i][j] = Math.min(dist1, dist2);      // similar to LCS
                }
            }
        }
        return DP[m][n];
...
class Solution 
{
    public int minDistance(String word1, String word2)  // Time: n*m, Space: Math.min(n, m)
    {
        int m = word1.length();
        int n = word2.length();
