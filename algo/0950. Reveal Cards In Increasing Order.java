...
/*
    ...
    Logic:
        Sort the deck
        Initially add all index to queue
        Fill the output by leaving a empty block --> fill, skip, fill, skip, ....
...
class Solution {
    public int[] deckRevealedIncreasing(int[] deck) {
        ...
        for(int i = 0; i < n; i++) {
            queue.add(i);                   // add all index to queue
        }
        
        int[] output = new int[n];
        for(int i = 0; i < n; i++) {
            output[queue.poll()] = deck[i]; // fill
            queue.add(queue.poll());        // skip
        }
        return output;
    }
}