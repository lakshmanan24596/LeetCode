...
/*
    Logic: dfs preorder traveral
    time: n
    space: n (for output list)
    
    Main implementation: Instead of flipping the node, just flip the recursion call
*/
class Solution {
    int index = 0;
    List<Integer> flipNodes;
    int[] voyage;
    
    public List<Integer> flipMatchVoyage(TreeNode root, int[] voyage) {
        this.voyage = voyage;
        flipNodes = new ArrayList<Integer>();
        dfs(root);
        
        if (!flipNodes.isEmpty() && flipNodes.get(0) == -1) {       // ex: convert [-1, 6] to [-1]
            flipNodes.clear();
            flipNodes.add(-1);
        }
        return flipNodes;
    ...
    public void dfs(TreeNode root) {
        ...
        if (root.val != voyage[index]) {        // example: 1
            flipNodes.clear();
            flipNodes.add(-1);
            return;
        }
        index++;
        if (index == voyage.length) {           // processed all nodes
            return;
        }
        ...
        if (root.left == null || root.left.val == voyage[index]) {
            ...
            flipNodes.add(root.val);
            dfs(root.right);                    // main logic: flip the recursive call
            dfs(root.left);
        }
    }  
}