...
/*
    time: constructor: O(1), set: O(1), get: O(log n)
    space: O(n)
    
    logic:
        set --> directly add a entry in hashMap --> O(1) time
        get --> binary search or treemap floor  --> O(log n) time
*/
class TimeMap {
    ...
    public void set(String key, String value, int timestamp) {
        ...
        if (currKeyData == null) {
            currKeyData = new TreeMap<Integer, String>();
            data.put(key, currKeyData);
        }
        currKeyData.put(timestamp, value);
    }
