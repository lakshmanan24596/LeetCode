...
Follow up: 
Can you find a logarithmic time complexity (i.e., O(log(n))) solution?
*/
...
class Solution {
    public int missingElement(int[] nums, int k) {
        ...
        while (left < right) {
            ...
            missingCount = (nums[mid] - nums[left]) - (mid - left);     // main logic
            
            if (missingCount >= k) {
                right = mid - 1;            // go left
            } else {
                k -= missingCount;
                left = mid;                 // reduce k and go right
            }
        }
        return nums[left] + k;
    }
