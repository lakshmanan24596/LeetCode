...
/*
    Logic: create a rank array and then sort it

    Time O(nm) for iterating
    Time O(m * mlogm) for sorting
    Space O(m^2)
...
class Solution {
    public String rankTeams(String[] votes) {
        String sampleVote = votes[0];
        int n = sampleVote.length();
        int[][] rank = new int[26][n];
        Character[] output = new Character[n];
        int charIndex;
        
        for (String vote : votes) {                                     // create rank array
            for (int i = 0; i < vote.length(); i++) {
                charIndex = vote.charAt(i) - 'A';
                rank[charIndex][i]++;
            }
        }
        for (int i = 0; i < n; i++) {                                   // create output array
            output[i] = sampleVote.charAt(i);
        }
        Arrays.sort(output, (ch1, ch2) -> {                             // sort output based on rank
            int index1 = ch1 - 'A';
            int index2 = ch2 - 'A';
            for (int i = 0; i < n; i++) {
                if (rank[index1][i] != rank[index2][i]) {
                    return rank[index2][i] - rank[index1][i];           // main logic for sorting (descending order)
                }
            }
            return index1 - index2;                                     // sort alphabetically
        });
        StringBuilder outputString = new StringBuilder();               // convert Character[] to string
        for (int i = 0; i < n; i++) {
