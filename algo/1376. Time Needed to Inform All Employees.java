...
class Solution {
    ArrayList<Integer>[] adjList;                              // n-ary tree to store parent-child relation
    int[] informTime;
    ...
    public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
        ...
        for (int i = 0; i < n; i++) {                          // create employee tree
            if (i != headID) {
                children = adjList[manager[i]];
                children.add(i);
            }
        }
        return dfs(headID);
    ...
    public int dfs(int currNode) {
        ...
        List<Integer> children = adjList[currNode];
        
        for (int child : children) {
            currOutput = informTime[currNode] + dfs(child);     // main logic
...
class Solution {
    ...
    public int dfs(int i) {
        if (manager[i] != -1) {
            informTime[i] += dfs(manager[i]);       // main logic
            manager[i] = -1;                        // main logic: to avoid solving it again
        }
        return informTime[i];
    }
}