...
class Solution {
    ...
    public int countRoutes(int[] locations, int start, int finish, int fuel) {
        this.noOfCity = locations.length;
        this.locations = locations;
        this.finish = finish;
        this.DP = new Integer[noOfCity][fuel + 1];
        return dfs(start, fuel);
    }
    
    public int dfs(int curr, int fuel) {
        if (DP[curr][fuel] != null) {
            return DP[curr][fuel];
        }
        int fuelNeeded;
        int output = (curr == finish) ? 1 : 0; // should not return, because you are allowed to visit more than once
        
        for (int next = 0; next < noOfCity; next++) {
            if (next != curr) {
                fuelNeeded = Math.abs(locations[curr] - locations[next]);
                if (fuel >= fuelNeeded) {
                    output += dfs(next, fuel - fuelNeeded);
                    output %= MOD;
                }
            }
        }
        return DP[curr][fuel] = output;
    }
}