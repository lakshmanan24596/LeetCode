...
/*
    ...
    Example:
        colors = "bbbaaa"                          // we need to remove 2 b's and 2 a's
        neededTime = [4,9,3,8,8,9]
        output = ((4+9+3) - 9) + ((8+8+9) - 9)      // main logic
               = 7 + 16
*/
...
class Solution {
    public int minCost(String colors, int[] neededTime) {
        int n = colors.length();
        int minCost = 0, maxCost, totalCost;
        ...
        for (int i = 1; i < n; ) {
            if (colors.charAt(i) != colors.charAt(i - 1)) {
                ...
                while (i < n && colors.charAt(i) == colors.charAt(i - 1)) {     // currColor == prevColor
                    maxCost = Math.max(maxCost, neededTime[i]);
                    totalCost += neededTime[i];
                    i++;
                }
                minCost += totalCost - maxCost;                                 // main logic
            }
        }
        return minCost;
    }
