...
/*
    greedy doesnt work, so DP
    sort the players by age
    then use DP technique similar to problem 300. Longest Increasing Subsequence
*/
...
class Solution {
    public int bestTeamScore(int[] scores, int[] ages) {
        ...
        Arrays.sort(player, (a, b) -> (a[1] == b[1]) ? (a[0] - b[0]) : (a[1] - b[1]));       // sort by age
        int[] DP = new int[n];
        int maxScore = Integer.MIN_VALUE, currScore;
        
        for (int i = 0; i < n; i++) {                       // similar to DP problem 300. Longest Increasing Subsequence
            DP[i] = player[i][0];
            for (int j = i - 1; j >= 0; j--) {
                if (player[i][0] >= player[j][0]) {         // main condition
                    currScore = player[i][0] + DP[j];       // use previous calculated result
                    DP[i] = Math.max(DP[i], currScore);
                }
            }
