...
/*
    ...
    3) time optimized using pre calculated best results by iterating L->R and R->L in each row
        time: r*c
        space: c
        
        consider 2, 7, 4:
            L->R = 2, 7, 6 and R->L = 6, 7, 4
            max(L->R, R->L) = 6, 7, 6
*/

class Solution {
    public long maxPoints(int[][] points) {     // both time and space optimized
        ...
        for (int i = 0; i < rows; i++) {
            ...
            if (i != 0) {
                prevRow[0] = currRow[0];
                for (int j = 1; j < cols; j++) {                            // L->R
                    prevRow[j] = Math.max(currRow[j], prevRow[j - 1] - 1);  // max(curr, left)
                }
                for (int j = cols - 2; j >= 0; j--) {                       // R->L
                    prevRow[j] = Math.max(prevRow[j], prevRow[j + 1] - 1);  // max((curr,left), right)
                }
            }
            
            // use prevRow[] data to calculate currRow[]
            for (int j = 0; j < cols; j++) {
                currRow[j] = prevRow[j] + points[i][j];
            }
        }
        
        long maxPoints = currRow[0];
        for (int i = 1; i < currRow.length; i++) {
            maxPoints = Math.max(maxPoints, currRow[i]);
        }
        return maxPoints;
    }
}
