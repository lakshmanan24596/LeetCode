...
The plants are arranged in a row and are labeled from 0 to n - 1 from left to right where the ith plant is located at x = i. There is a river at x = -1 that you can refill your watering can at.
...
class Solution {
    public int wateringPlants(int[] plants, int capacity) {
        ...
        for (int i = 0; i < plants.length; i++) {
            if (plants[i] > currCapacity) {     // main logic
                currCapacity = capacity;
                steps += i * 2;                 // go to river and come back to its original position 
            }
